...
def pad_sents_char(sents, char_pad_token):
    ...
    max_sentence_len = len(max(sents, key=lambda sent: len(sent)))
    sents_padded = []
    for i in range(len(sents)):
        sent_char_padded = [(w + [char_pad_token] * (max_word_length - len(w))) for w in sents[i]]
        sents_padded.append(sent_char_padded + [[char_pad_token] * max_word_length for _ in range(max_sentence_len - len(sents[i]))])
    ### TODO:
