...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2, no_char_decoder=False):
        ...
        self.target_vocab_projection = nn.Linear(in_features=hidden_size,
                                                 ...
                                                 bias=False)
        self.dropout = nn.Dropout(p=0.3)  # set p=0 to pass sanity check
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        packed_sequence = self.encoder(pack_padded_sequence(X, source_lengths))  # pack to set <pad> indices words zero embedding after RNN
        enc_hiddens, last_hidden, last_cell = pad_packed_sequence(packed_sequence[0])[0].permute(1, 0, 2), \
                                              packed_sequence[1][0], \
                                              packed_sequence[1][1]  # (b, src_len, h*2), (2, b, h), (2, b, h)
        init_decoder_hidden = self.h_projection(last_hidden.permute(1, 0, 2).reshape(-1, 2*self.hidden_size))  # (b, h), apply reshape only doesn't work
        init_decoder_cell = self.c_projection(last_cell.permute(1, 0, 2).reshape(-1, 2*self.hidden_size))  # (b, h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            o_prev = o_t
        combined_outputs = torch.stack(combined_outputs, 0)  # (tgt_len, b, h)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        src_sents_var = self.vocab.src.to_input_tensor_char([src_sent], self.device)  # (11, 1, 21) (sentence_len, b, word_len)

        src_encodings, dec_init_vec = self.encode(src_sents_var, [len(src_sent)])  # (1, 11, 512) 用char_emb得到word_emb，并过rnn得到每个word的hidden units，构成src_encodings
        src_encodings_att_linear = self.att_projection(src_encodings)  # (1, 11, 256) 之后和decoder的hidden units点积计算attention分数
        ...
        hypotheses = [['<s>']]  # 不同分支的hypotheses，每个分支是以<s>开头的list(str)
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            y_tm1 = self.vocab.tgt.to_input_tensor_char(list([hyp[-1]] for hyp in hypotheses), device=self.device)  # (1, hyp_num, 21) (sentence_len, b, word_len)
            y_t_embed = self.model_embeddings_target(y_tm1)  # (1, hyp_num, emb_size) tgt输入的word_emb
            y_t_embed = torch.squeeze(y_t_embed, dim=0)

            x = torch.cat([y_t_embed, att_tm1], dim=-1)

            (h_t, cell_t), att_t, _  = self.step(x, h_tm1,
                                                      exp_src_encodings, exp_src_encodings_att_linear, enc_masks=None)  # decoder得到每个step的hidden units

            # log probabilities over target words
            log_p_t = F.log_softmax(self.target_vocab_projection(att_t), dim=-1)  # (hyp_num=b, word_vocab_size=32)

            live_hyp_num = beam_size - len(completed_hypotheses)
            contiuating_hyp_scores = (hyp_scores.unsqueeze(1).expand_as(log_p_t) + log_p_t).view(-1)  # !!!(hyp_num * word_vocab_size), 即各路各个单词的分数汇总，取topK
            top_cand_hyp_scores, top_cand_hyp_pos = torch.topk(contiuating_hyp_scores, k=live_hyp_num)  # (5,)

            prev_hyp_ids = top_cand_hyp_pos // len(self.vocab.tgt)  # 因为line 356合并了前两维，这里拆开
            ...
            for prev_hyp_id, hyp_word_id, cand_new_hyp_score in zip(prev_hyp_ids, hyp_word_ids, top_cand_hyp_scores):
                ...
                if hyp_word == "<unk>":
                   hyp_word = "<unk>"+str(len(decoderStatesForUNKsHere))  #后缀为了拼接时找到decodedWords中具体的word
            ...
            if len(decoderStatesForUNKsHere) > 0 and self.charDecoder is not None: # decode UNKs
                decoderStatesForUNKsHere = torch.stack(decoderStatesForUNKsHere, dim=0)  # (num_unks=batch in char decoder, hidden_size)
